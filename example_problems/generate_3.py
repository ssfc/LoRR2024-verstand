...
def modify_tasks_file(input_file):
    ...
    # 根据输入文件名动态生成输出文件名
    output_file = os.path.join(input_dir, f"{filename_without_ext}_3_errands{ext}")  # 新的输出文件路径
    ...
    for num1, num2 in parsed_lines:
        ...
        if count % 400 == 0:
            print(count)
        valid_numbers = list(all_numbers - {num1, num2})  # 从所有数字中去掉本行的两个数字
        if not valid_numbers:
            raise ValueError("无法找到符合条件的数字")
        third_num = random.choice(valid_numbers)  # 随机选择一个有效数字
        modified_lines.append(f"{num1},{num2},{third_num}")
...
def main():
    parser = argparse.ArgumentParser(description="Process a .tasks file and add a third number.")
