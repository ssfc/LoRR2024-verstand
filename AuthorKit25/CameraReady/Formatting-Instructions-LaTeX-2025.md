...
### Q: Your title must follow Title Case capitalization rules (not sentence case).  这句话怎么理解？实词大写吗？
...
`aaai25.sty` 文件与 `hyperref` 和 `navigator` 软件包不兼容。如果您使用了其中任一个，您的参考文献将会出现错误并导致您的文章被退回。（comment:  怪不得不让用hyperref）
